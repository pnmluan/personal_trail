angular.module('MetronicApp').controller('ContactController', function($rootScope, $scope, $http, $base64, $timeout, $location, $q, ContactService, ngDialog, toastr, DTOptionsBuilder, DTColumnBuilder, Upload) {
    ...
    $scope.clickToAddNew = function() {
        ...
        ngDialog.openConfirm({
            template: 'views/contact/model_form_contact.html',
            ...
            controller: ['$scope', 'data', function($scope, data){
                ...
                $scope.save = function() {
                    ...
                    ContactService.save($scope.mItem).then(function(res) {
    ...
    function getItemByID(id) {
        ...
        var item = {};
        ContactService.getCategories().then(function(res) {
    ...
    $scope.clickToUpdate = function(item) {
        ngDialog.openConfirm({
            ...
            controller: ['$scope', '$filter', 'data', function($scope, $filter, data){
                ...
                //Load Image
                $scope.img = $scope.settings.imgPath + 'contact/' + item.img; 

                // Create contact
                $scope.save = function() {
                    $scope.mItem.status = $scope.optionStatus.selected.id;
                    ContactService.save($scope.mItem, $scope.mItem.id).then(function(res) {
    ...
    $scope.clickToDelete = function(id) {
        swal({
          ...
          buttonsStyling: false
            ...
            ContactService.delete(id).then(function(res) {
                if(res.status == 200) {
