angular.module('MetronicApp').controller('TagController', function($rootScope, $scope, $http, $base64, $timeout, $location, $q, TagService, ngDialog, toastr, DTOptionsBuilder, DTColumnBuilder, Upload) {
    ...
    $scope.clickToAddNew = function() {
        ...
        ngDialog.openConfirm({
            template: 'views/tag/model_form_tag.html',
            ...
            controller: ['$scope', 'data', function($scope, data){
                ...
                $scope.save = function() {
                    ...
                    TagService.save($scope.mItem).then(function(res) {
    ...
    function getItemByID(id) {
        ...
        var item = {};
        TagService.getCategories().then(function(res) {
    ...
    $scope.clickToUpdate = function(item) {
        ngDialog.openConfirm({
            ...
            controller: ['$scope', '$filter', 'data', function($scope, $filter, data){
                ...
                //Load Image
                $scope.img = $scope.settings.imgPath + 'tag/' + item.img; 

                // Create tag
                $scope.save = function() {
                    $scope.mItem.status = $scope.optionStatus.selected.id;
                    TagService.save($scope.mItem, $scope.mItem.id).then(function(res) {
    ...
    $scope.clickToDelete = function(id) {
        swal({
          ...
          buttonsStyling: false
            ...
            TagService.delete(id).then(function(res) {
                if(res.status == 200) {
